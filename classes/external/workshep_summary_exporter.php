...
/**
 * Class for exporting partial workshep data.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class workshep_summary_exporter extends exporter {
    ...
    protected static function define_properties() {
        ...
        return array(
            ...
            'course' => array(
                'type' => PARAM_INT,
                'description' => 'Course id this workshep is part of.',
            ),
            'name' => array(
                'type' => PARAM_TEXT,
            ...
            'phase' => array(
                'type' => PARAM_INT,
                'default' => 0,
                'description' => 'The current phase of workshep (0 = not available, 1 = submission, 2 = assessment, 3 = closed).',
                'optional' => true,
            ),
            'useexamples' => array(
                'type' => PARAM_BOOL,
            ...
            'strategy' => array(
                'type' => PARAM_PLUGIN,
                'description' => 'The type of the current grading strategy used in this workshep.',
                'optional' => true,
            ),
            'evaluation' => array(
                'type' => PARAM_PLUGIN,
            ...
            'conclusion' => array(
                'type' => PARAM_RAW,
                'description' => 'A text to be displayed at the end of the workshep.',
                'optional' => true,
                'null' => NULL_ALLOWED,
            ),
    ...
    protected function get_other_values(renderer_base $output) {
        ...
        $values['introfiles'] = external_util::get_area_files($context->id, 'mod_workshep', 'intro', false, false);

        if (!empty($this->data->instructauthors)) {
            $values['instructauthorsfiles'] = external_util::get_area_files($context->id, 'mod_workshep', 'instructauthors');
        }

        if (!empty($this->data->instructreviewers)) {
            $values['instructreviewersfiles'] = external_util::get_area_files($context->id, 'mod_workshep', 'instructreviewers');
        }

        if (!empty($this->data->conclusion)) {
            $values['conclusionfiles'] = external_util::get_area_files($context->id, 'mod_workshep', 'conclusion');
        }
